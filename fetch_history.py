...
def main():
    print("🚀 Сбор исторических данных Binance Futures (6 месяцев)...")
    end_time = int(datetime.now().timestamp() * 1000)
    start_time = int((datetime.now() - timedelta(days=DAYS_BACK)).timestamp() * 1000)
    
    print("Загрузка поводыря (BTCUSDT)...")
    ...
    for symbol in tqdm(SYMBOLS, desc="Обработка монет"):
        ...
        df = df.dropna().iloc[:-40] 
        all_data.append(df)
        
    final_df = pd.concat(all_data, ignore_index=True)
    
    cols_to_save = ['timestamp', 'symbol', 'rsi', 'macd', 'adx', 'ema9_dist_pct', 'volatility_pct', 'cvd_15m', 'btc_trend', 'target_long', 'target_short']
    final_df[cols_to_save].to_csv("deep_history_dataset.csv", index=False)
    
    print(f"\n✅ Готово! Собрано {len(final_df)} строк качественных данных.")
